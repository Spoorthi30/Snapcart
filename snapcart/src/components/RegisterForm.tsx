...
const RegisterForm = ({prevStep}:propType) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center w-full min-h-screen relative bg-linear-to-b from-green-200 to-white'>
      <div className='flex items-center gap-2 text-green-600 font-semibold absolute top-6 left-6 transition-colors cursor-pointer hover:text-green-700' onClick={()=>prevStep(1)}>
      ...
      className='text-green-600 text-2xl md:text-3xl font-bold'>Create Account</motion.h2>
      ...
      <form className='flex flex-col gap-6 max-w-sm w-full' onSubmit={handleSubmit}>
        <div className='relative'>
          <User className='absolute top-3 left-3.5 w-5 h-5 text-gray-400'/>
          <input type='text' placeholder='Your Name' className='w-full border border-gray-300 rounded-xl py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-800'
        ...
        <div className='w-full border border-gray-400 text-gray-700 py-3 font-semibold rounded-2xl flex items-center justify-center gap-6 cursor-pointer' onClick={()=>signIn("github",{callbackUrl:"/"})}>
          <FaGithub />Continue with Github
        </div>
      </form>
        <div className='flex items-center mt-4 text-sm font-medium'>Already have an account ? 
          <span className='flex items-center text-green-700 font-semibold'><LogIn className='w-4 h-4 mx-1'/> <Link href='/login'>Sign In</Link></span>
        </div> 
    </div>
  )
}
